...
This is a small extension of the POC's hosted Checkout path. It is sufficient to collect a payment, retain a durable business record, and trigger a manual or automated downstream process. Customer billing management, payment-method display, subscriptions, invoices, refunds, and order history can be added later through Stripe APIs and the Stripe Customer Portal.
...
6. The server creates a hosted Stripe Checkout Session and redirects the user to Stripe.
7. Stripe returns the user to an application confirmation page.
...
sequenceDiagram
    ...
    App->>Stripe: Create Checkout Session with server-selected Price
    Stripe-->>App: Session ID and hosted URL
    App->>DB: Save Stripe Checkout Session ID
    App-->>User: Redirect to secure Stripe Checkout
    User->>Stripe: Complete or abandon payment
...
The recommended immediate investment is **Fix Now MVP using Hosted Checkout**. It builds on the most reliable, lowest-code POC path, preserves Stripe as the system that stores payment methods, and adds only the application records needed to connect a confirmed payment to a downstream action.
